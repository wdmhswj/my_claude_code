"""
s07: Skill loading - 两级按需的知识注入

    Layer 1 (cheap, 总是存在):
        SYSTEM prompt 中包含 skill names + 一行的 descriptions (~100 tokens/skill)
        "Skill available: agent-builder, code-review, mcp-builder, pdf"
    
    Layer 2 (expensive, on demand):
        Agent 调用 load_skill("code_reivew") -> 完整的 SKILL.md 内容
...
import ast, os, json, subprocess, yaml
...
if __name__ == "__main__":
    print("s07: SKill loading - 在 SYSTEM prompt 加载 skill 目录, 按需加载 skill 内容")
